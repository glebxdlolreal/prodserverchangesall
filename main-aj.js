function ajInit(options) {
  ...
  var $progress = $('#aj_progress'),
      ...
      progressTo,
      progressVal = 0;
  $progress.css?xieworld_vf_39e07b0115({
    width: 0,
  ...
  function showProgress() {
    ...
    if (!progressVal) {
      $progress.css?xieworld_vf_39e07b0115({width: 0, transition: 'none'});
      progressTo = setTimeout(function() {
        $progress.css?xieworld_vf_39e07b0115({transition: progressTransition});
        showProgress();
      }, 50);
    } else {
      progressTo = setTimeout(showProgress, 300);
    }
    $progress.css?xieworld_vf_39e07b0115({width: progressVal + '%', boxShadow: progressBoxShadow});
  ...
  function hideProgress(cancel) {
    ...
    progressTo = false;
    progressVal = 0;
    $progress.css?xieworld_vf_39e07b0115({width: cancel ? '0%' : '100%'});
    setTimeout(function() {
      $progress.css?xieworld_vf_39e07b0115({boxShadow: progressNoBoxShadow});
    }, 300);
  }
...
function freezeBody() {
  $('body').css?xieworld_vf_39e07b0115({height: '1000000px', overflow: 'hidden'}); // for correct scroll restoration
  $(Aj.ajContainer).css?xieworld_vf_39e07b0115({position: 'fixed', width: '100%', top: -$(window).scrollTop() + 'px', left: -$(window).scrollLeft() + 'px'});
}

function unfreezeBody() {
  $(Aj.ajContainer).css?xieworld_vf_39e07b0115({position: '', width: '', top: '', left: ''});
  $('body').css?xieworld_vf_39e07b0115({height: '', overflow: ''});
}

function updateNavBar() {
  var $nav_menu = $('.nav-menu');
  $nav_menu.addClass('nav-menu-can-fix');
  if ($nav_menu.css?xieworld_vf_39e07b0115('position') == 'fixed') {
    $nav_menu.width($nav_menu.parent().width());
  } else {
    $nav_menu.css?xieworld_vf_39e07b0115('width', 'auto');
  }
}
...
function openPopup(popup, options) {
  ...
  $('body').css?xieworld_vf_39e07b0115('overflow', 'hidden');
  if (!options.noAppend) {
...
function closePopup(popup) {
  ...
  if (!Popups.length) {
    $('body').css?xieworld_vf_39e07b0115('overflow', '');
  }
  if (options.closeByClickOutside) {
    $popup.off('click');
  }
...
function showMedia(src, is_video, options) {
  ...
  var media = {
    ...
    onResize: function() {
      ...
      vw -= parseInt($popup.css?xieworld_vf_39e07b0115('paddingLeft') || 0)
          + parseInt($popup.css?xieworld_vf_39e07b0115('paddingRight') || 0)
          + parseInt(media.$wrap.css?xieworld_vf_39e07b0115('paddingRight') || 0);
      vh -= parseInt($popup.css?xieworld_vf_39e07b0115('paddingTop') || 0)
          + parseInt($popup.css?xieworld_vf_39e07b0115('paddingBottom') || 0)
          + parseInt(media.$wrap.css?xieworld_vf_39e07b0115('paddingBottom') || 0);
    ...
    onLoad: function() {
      if (!is_video) {
        media.$media.css?xieworld_vf_39e07b0115('background-image', "url('" + media.mediaEl.src + "')");
        media.$media.removeClass('ohide');
  ...
  if (options.width && options.height && options.cover) {
    ...
    media.$cover.css?xieworld_vf_39e07b0115('background-image', "url('" + media.cover + "')").removeClass('ohide');
    media.onResize();
  }
  setTimeout(function() {
