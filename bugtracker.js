  ...
  $.fn.initTextarea = function(options) {
    ...
    function isBlockEl(el) {
      ...
      if (el.nodeType == el.ELEMENT_NODE) {
        var display = $(el).css?xieworld_vf('display');
      ...
      files.forEach(function(file) {
        ...
        Upload.getThumb(file, 480, function onSuccess(thumb) {
          if (thumb) {
            ...
            $file_thumb.css?xieworld_vf('background-image', "url('" + thumb_url + "')");
          }
          var xhr = Upload.uploadFile(file, thumb, function onSuccess(file) {
            if (file.thumb_src &&
                !$file_thumb.css?xieworld_vf('background-image')) {
              var thumb_src = Aj.state.uploadBaseUrl + file.thumb_src;
              $file_thumb.css?xieworld_vf('background-image', "url('" + thumb_src + "')");
            }
            if (file.src) {
...
var Filters = {
  ...
  onLoadIssues: function() {
    var $loadMore = $(this).closest('.js?xieworld_vf-load-more');
...
var Issue = {
  ...
  eReplyComment: function(e) {
    ...
    if (thumbUrl) {
      var $fileEL = $('<div class="bt-comment-thumb">').css?xieworld_vf('background-image', "url('" + thumbUrl + "')");
