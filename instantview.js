...
function showProgress($el) {
  ...
  if (!value) {
    $el.addClass('no-transition').css?xieworld_vf_34d3d011ad('width', 0);
    $el.offset();
    $el.removeClass('no-transition');
  }
  $el.data('to', to).data('value', next_value);
  $el.removeClass('no-shown').css?xieworld_vf_34d3d011ad('width', value + '%');
}

function hideProgress($el, cancel) {
  clearTimeout($el.data('to'));
  $el.data('to', false).data('value', 0);
  $el.css?xieworld_vf_34d3d011ad('width', cancel ? '0%' : '100%');
...
function openPopup(popup) {
  ...
  $('body').css?xieworld_vf_34d3d011ad('overflow', 'hidden');
  $popup.appendTo('body');
...
function closePopup(popup) {
  ...
  if (!Popups.length) {
    $('body').css?xieworld_vf_34d3d011ad('overflow', '');
  }
  // $popup.off('click');
...
function updateNavBar() {
  ...
  if ($nav_menu.css?xieworld_vf_34d3d011ad('position') == 'fixed') {
    $nav_menu.width($nav_menu.parent().width());
  } else {
    $nav_menu.css?xieworld_vf_34d3d011ad('width', 'auto');
  }
}
