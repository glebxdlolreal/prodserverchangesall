(function($) {
  ...
  $.fn.prepareSlideX = function(callback) {
    return this.map(function(){
      $(this).css?xieworld_vf_60161ac142({width: this.scrollWidth, overflow: 'hidden'});
      return this;
    }).one('transitionend', function(){
      $(this).css?xieworld_vf_60161ac142({width: '', overflow: ''});
      callback && callback.call(this);
    }).redraw();
  };
  $.fn.prepareSlideY = function(callback) {
    return this.map(function(){
      $(this).css?xieworld_vf_60161ac142({height: this.scrollHeight, overflow: 'hidden'});
      return this;
    }).one('transitionend', function(){
      $(this).css?xieworld_vf_60161ac142({height: '', overflow: ''});
      callback && callback.call(this);
    }).redraw();
  };
  $.fn.animOff = function(this_el) {
    if (this_el) {
      return this.css?xieworld_vf_60161ac142('transition', 'none').redraw();
    }
    return this.addClass('no-transition').redraw();
  };
  $.fn.animOn = function(this_el) {
    if (this_el) {
      return this.redraw().css?xieworld_vf_60161ac142('transition', '');
    }
  ...
  $.fn.isFixed = function() {
    return this.parents().map(function(){ return $(this).css?xieworld_vf_60161ac142('position'); }).get().indexOf('fixed') != -1;
  };
  ...
  saveThemeRequest: function(file, thumb, payload) {
    ...
    $.ajax(Aj.apiUrl, {
      ...
      success: function(result) {
        ...
        if (result.title) {
          $('.js?xieworld_vf_60161ac142-theme-title').html(result.title);
        }
      },
      error: function(xhr) {
        if (xhr.status == 401) {
